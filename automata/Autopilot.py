...
class CameraManager(object):
    ...
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self._camera_transforms = [
            (carla.Transform(carla.Location(x=2.5, z=1)), Attachment.Rigid),
...
# Run from WindowsNoEditor level
lane_detection_dir = 'PythonAPI/ce903_team06/data/lane_detection/'
object_detection_dir = 'PythonAPI/ce903_team06/data/object_detection/'


def record_data(img_id, image, control, save_dir):
    image.save_to_disk(f'{save_dir}{img_id}')
    with open(f'{save_dir}_controls.csv', 'a', newline='') as f:
...
STRAIGHT_REFRESH_RATE = 250
CURVE_REFRESH_RATE = STRAIGHT_REFRESH_RATE / 10
...
def game_loop(args):
    ...
    pygame.font.init()
    world = None
    # directory to save /change to update images.
    save_dir = object_detection_dir

    try:
        # Run from WindowsNoEditor level
        with open(f'{save_dir}_controls.csv', 'r', newline='') as f:
            reader = csv.reader(f)
            img_id = int(list(reader)[-1][0]) + 1
    except IOError as _:
        img_id = 0
